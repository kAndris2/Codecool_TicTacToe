...
def Choose_Zone(p, char, b=False):
    global player
    zone = input("\n" + get_Name(p) +  " [" + char + "]: ")
    if len(zone) < 2:
        Print_Board()
...
def Choose(sor, oszlop, char):
    ...
    if board[sor][oszlop] == "N":
        ...
        return Choose_Zone(GetID(char), char, True)

def ChooseSingle(sor, oszlop, char):
    global board
    sor -= 1
    oszlop -= 1
    if board[sor][oszlop] == "N":
        ...
        Print_Board()
        return Choose_Zone_Computer(GetID(char), char, True)
...
def Win(p):
    print("\n" + get_Name(p) +  " [" + get_Char(p) + "] is the winner!")
...
def Start_Check():
    ...
    while True:
        try:
            size = int(input("Size of the map (3-26): "))
        except ValueError:
            os.system("clear")
            print(CRED, "Only numbers!", CEND)
        else:
            break
    if size > 26 or size < 3:
        os.system("clear")
        print(CRED, "Choose a number between 3 and 26!\n", CEND)
        return Start_Check()
...
def AskIfBackToMenu(): #vissza a menube kerdes
    while True:
        backtomenu = str(input("\nPress[Enter]\n"))
        backtomenu = backtomenu.lower()
        if backtomenu == "y" or backtomenu == "":
            os.system("clear")
            Call_Menu()
        elif backtomenu == "n":
            os.system("clear")
...
def Choose_Zone_Computer(p, char, b=False):
    global player
    zone = ""
    x = random.randint(0, size)
    n = chr(random.randint(65, (65+size)))
    zone += n + str(x)

    if len(zone) < 2:
        Print_Board()
        return Choose_Zone_Computer(p, char)
    zoneid = get_RowColumn(zone)
    #
    if int(zoneid[0]) == 0:
        Print_Board()
    ...
    if ord(zoneid[1]) > max:
        ...
        return Choose_Zone_Computer(p, char)
    #
    ChooseSingle(int(zoneid[0]), IndexColumn(zoneid[1]), char)
    Print_Board()
    global round
    round += 1
...
def Play_Singleplayer():
    ...
    while True:
        Print_Board()
        Choose_Zone_Computer(1, get_Char(1))

        Print_Board()
        Choose_Zone_Computer(2, get_Char(2))

x = threading.Thread(target=PlayMusic, daemon=True)
y = threading.Thread(target=Call_Menu)
x.start()
Startup()
y.start()
